...
class builder_cares(builder_base):
    def __init__(self, env:environment.base):
        super().__init__("c-ares", env)
        ...
        self.env.run_commands(
            commands = [
                f'cmake -B "{self.module_build_dir.as_posix()}"'
                    ...
                    f' -DCARES_STATIC={build_static}'
                    f' -DCARES_SHARED={build_shared}'
                    f' -DCARES_BUILD_TOOLS=OFF'
                    f' -DCARES_BUILD_TESTS=OFF'
                    ,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Build and install c-ares")
